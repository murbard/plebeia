...
val root : context -> hash -> (('iprev_, 'hprev_, 'ihole_, 'hhole_, 'modified_) cursor, error) result
(** Gets the root cursor corresponding to a given root hash in the
    context. *)

val empty : context -> ('iprev_, 'hprev_, indexed, hashed, unmodified) cursor
(** Creates a cursor to a new, empty tree. *)

val subtree : ('iprev, 'hprev, 'ihole, 'hhole, 'modified) cursor -> segment -> (('iprev_, 'hprev_, 'ihole_, 'hhole_, 'modified_) cursor, error) result
(** Moves the cursor down a segment, to the root of a sub-tree. Think
    "cd segment/" *)

val parent : ('iprev, 'hprev, 'ihole, 'hhole, 'modified) cursor -> ('iprev_, 'hprev_, 'ihole_, 'hhole_, 'modified_)  cursor
(** Moves the cursor back to the parent tree. Think "cd .." *)

val get : ('iprev, 'hprev, 'ihole, 'hhole, 'modified) cursor -> segment -> (value, error) result
(** Gets a value if present in the current tree at the given
    segment. *)

val insert: ('iprev, 'hprev, 'ihole, 'hhole, 'modified) cursor -> segment -> value -> (('iprev_, 'hprev_, 'ihole_, 'hhole_, 'modified_) cursor, error) result
(** Inserts a value at the given segment in the current tree.
    Returns the new cursor if successful. *)

val update: ('iprev, 'hprev, 'ihole, 'hhole, 'modified) cursor -> segment -> value -> ('iprev_, 'hprev_, 'ihole_, 'hhole_, 'modified_) cursor
(** Updates a value at the given segment in the current tree.
    Returns the new cursor if successful. *)

val upsert: (('iprev_hole * 'iprev_trail), ('hprev_hole * 'hprev_trail), 'ihole, 'hhole, 'modified) cursor -> segment -> value -> ((('iprev_hole_ * 'iprev_trail), ('hprev_hole * 'hprev_trail), not_indexed, not_hashed, modified) cursor, error) result
(** Upserts. This can still fail if the segment leads to a subtree. *)

val snapshot: ('iprev, 'hprev, 'ihole, 'hhole, 'modified) cursor -> segment -> segment -> (('iprev_, 'hprev_, 'ihole_, 'hhole_, 'modified_) cursor, error) result
(** Snapshots a subtree at segment and place a soft link to it at
    another segment location. *)

val commit: ('iprev, 'hprev, 'ihole, 'hhole, 'modified) cursor -> hash
(** Commits the change made in a cursor to disk. Returns the new root
    hash. *)

val hash: ('iprev, 'hprev, 'ihole, 'hhole, 'modified) cursor -> hash
